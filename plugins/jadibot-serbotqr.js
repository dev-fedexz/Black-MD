...
msgRetry,
msgRetryCache,
browser: ['Shadow-bot (Sub Bot)', 'Chrome','2.0.0'],
version: version,
...
if (qr) { 
if (m?.chat) {
txtQR = await conn.sendMessage(m.chat, { image: await qrcode.toBuffer(qr, { scale: 8 }), caption: rtx.trim()}, { quoted: m}) // Usa rtx
} else {
return 
}
if (txtQR && txtQR.key) {
...
if (options.fromCommand) m?.chat ? await conn.sendMessage(`${path.basename(pathShadowJadiBot)}@s.whatsapp.net`, {text : '*Reenvíe el comando*' }, { quoted: m || null }) : ""
