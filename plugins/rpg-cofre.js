const handler = async (m, { isPrems, conn }) => {
  if (!global.db.data.users[m.sender]) {
    throw `${emoji} Usuario no encontrado.`;
  }
  ...
  if (Date.now() < timeToNextCofre) {
    ...
    const mensajeEspera = `${emoji} Ya reclamaste tu cofre\n⏰️ Regresa en: *${msToTime(tiempoRestante)}* para volver a reclamar.`;
    await conn.sendMessage(m.chat, { text: mensajeEspera }, { quoted: m });
    return;
  }

  const img = 'https://raw.githubusercontent.com/El-brayan502/dat1/main/uploads/3cf55e-1762058680589.jpg';
...
╰━━━━━━━━━━━━⬣`;
  ...
  try {
    await conn.sendFile(m.chat, img, 'thumbnail.jpg', texto, fkontak);
  } catch (error) {
